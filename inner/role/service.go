...
func (serv *Service) Save(req Request) (id int64, err error) {
	...
	if err != nil {
		return 0, common.DbOperationError{Message: fmt.Sprintf("error save role: %w", err)}
	}
...
func (serv *Service) FindById(id int64) (Response, error) {
	...
	if err != nil {
		return Response{}, common.DbOperationError{Message: fmt.Sprintf("error finding role with id %d: %w", id, err)}
	}
...
func (serv *Service) GetAll() ([]Response, error) {
	...
	if err != nil {
		return []Response{}, common.DbOperationError{Message: fmt.Sprintf("error GetAll roles: %w", err)}
	}
...
func (serv *Service) FindByIds(ids []int64) ([]Response, error) {
	...
	if err != nil {
		return []Response{}, common.DbOperationError{Message: fmt.Sprintf("error finding role with ids %d: %w", ids, err)}
	}
...
func (serv *Service) DeleteById(id int64) error {
	...
	if err != nil {
		return common.DbOperationError{Message: fmt.Sprintf("error delete role by id %d: %w", id, err)}
	}
...
func (serv *Service) DeleteByIds(ids []int64) error {
	...
	if err != nil {
		return common.DbOperationError{Message: fmt.Sprintf("error delete role by ids %d: %w", ids, err)}
	}
